...
class Growth_NonPeriodic:
    ...
    def __init__(self, x_dim, y_dim, n_nano, KbT, mu, e_nn, e_nl, e_ll, nano_mob, nano_size, seed):
        ...
        self.attraction_x = 0.5
        self.attraction_e = 0.04
        ...
        self.norm = 1/math.sqrt(2)
        self.tot_E_norm = 1/(1+1/math.sqrt(2))
    ...
    def step_fluid(self):
        ...
        if self.nano[x_i,y_i] == 0:
            ...
            delta_e = -(1-2*self.fluid[x_i,y_i])*((self.tot_E_norm)*(self.e_ll*(self.fluid[(x_i-1),y_i]+
    ...
    '''
    def neigh_de(self,cur_cell,cell,dis):
        #x_i = cell[0]
        #y_i = cell[1]
        ...
        #(1-2*fluid), (1-2*nano)
        fluid_neigh = self.fluid[x_i_neigh,y_i_neigh]
        nano_neigh = self.nano[x_i_neigh,y_i_neigh]
        de = (-cur_cell)*(self.e_ll*(fluid_neigh)+\
                          self.e_nl*(nano_neigh))+\
             (cur_cell)*(self.e_nn*(nano_neigh)+\
                         self.e_nl*(fluid_neigh))
        return -de
        ...
        for i in range(1,self.x_dim-1):
            for j in range(1,self.y_dim-1):
                for k in range(4):
                    ...
                    x_n = (i+neigh[0])
                    y_n = (j+neigh[1])
                    total_e -= self.fluid[i,j]*                                ((1/(1+1/math.sqrt(2)))*(self.e_ll*(self.fluid[x,y]+(1/math.sqrt(2))*self.fluid[x_n,y_n])/2 +                                 self.e_nl*(self.nano[x,y]+(1/math.sqrt(2))*self.nano[x_n,y_n])) +                                 self.mu/4)+                               self.nano[i,j]*                                 ((1/(1+1/math.sqrt(2)))*self.e_nn*(self.nano[x,y]+(1/math.sqrt(2))*self.nano[x_n,y_n])/2)
        return total_e
    ...
    def step_nano(self):
        ...
        for i in range(self.nano_size):
            fluid_sum += self.fluid[(x_nano+offset[0]+i*abs_dis[1]),(y_nano+offset[1]+i*abs_dis[0])]
            nano_sum += self.nano[(x_nano+offset[0]+i*abs_dis[1]),(y_nano+offset[1]+i*abs_dis[0])]
        ...
        if not hitBoundary and fluid_sum == self.nano_size and nano_sum == 0:
            ...
            for i in range(self.nano_size):
                ...
                x_i = (x+i*abs_dis[1])
                y_i = (y+i*abs_dis[0])
                x_i_wake = (x_wake+i*abs_dis[1])
                y_i_wake = (y_wake+i*abs_dis[0])
                
                #Add needed bond contributions - nearest neighbours
                delta_e += self.neigh_de(1,(x_i,y_i),dis)
                delta_e += self.neigh_de(-1,(x_i_wake,y_i_wake),(-dis[0],-dis[1]))
                
                #Second nearest neighbours
                nnn_e += self.neigh_de(1,(x_i,y_i),(dis[0]+dis[1],dis[1]+dis[0]))
                nnn_e += self.neigh_de(1,(x_i,y_i),(dis[0]-dis[1],dis[1]-dis[0]))
                
                nnn_e += self.neigh_de(-1,(x_i_wake,y_i_wake),(-dis[0]+dis[1],-dis[1]+dis[0]))                
                nnn_e += self.neigh_de(-1,(x_i_wake,y_i_wake),(-dis[0]-dis[1],-dis[1]-dis[0]))
                
                if i == 0:
                    delta_e += self.neigh_de(1,(x_i,y_i),(-abs_dis[1],-abs_dis[0]))
                    delta_e += self.neigh_de(-1,(x_i_wake,y_i_wake),(-abs_dis[1],-abs_dis[0]))
                    
                    nnn_e += self.neigh_de(1,(x_i,y_i),(-dis[0]-abs_dis[1],-dis[1]-abs_dis[0]))
                    nnn_e += self.neigh_de(-1,(x_i_wake,y_i_wake),(dis[0]-abs_dis[1],dis[1]-abs_dis[0]))                    
                    ...
                    nnn_e += self.neigh_de(1,(x_i,y_i),(-dis[0]+abs_dis[1],-dis[1]+abs_dis[0]))
                    nnn_e += self.neigh_de(-1,(x_i_wake,y_i_wake),(dis[0]+abs_dis[1],dis[1]+abs_dis[0]))
            delta_e += (self.norm)*nnn_e 
            delta_e *= (self.tot_E_norm)
            #Calculate probability of accepting move
            att_e = self.attraction_energy(x_nano,x_nano+dis[0])


            Pacc = min(1,np.exp(-(delta_e+att_e)/self.KbT))
            if np.random.random() <= Pacc:
                #Accept 
                self.total_energy += delta_e
                #Move nanoparticle
                for i in range(self.nano_size):
                    x_i = (x_nano + offset[0] + i*abs_dis[1])
                    y_i = (y_nano + offset[1] + i*abs_dis[0])
                    x_i_wake = (x_nano + wake_offset[0] + i*abs_dis[1])
                    y_i_wake = (y_nano + wake_offset[1] + i*abs_dis[0])
                    self.fluid[x_i,y_i] = (1-self.fluid[x_i,y_i])
        ...
        return delta_e
    #Function for attraction energy towards a line at x = att_x (in lattice coordinate)
    #Attraction energy falls off as 1/(0.2+dist)
    def attraction_energy(self,prev_x,next_x):
        att_x = int(self.attraction_x*self.x_dim)
        prev_dist = abs(prev_x-att_x)
        next_dist = abs(next_x-att_x)
        return self.attraction_e*(next_dist-prev_dist)/(0.2+(prev_dist/self.x_dim)*self.x_length)
        ...
        for i in range(self.n_nano):
            ...
            while not isdone:
                ...
                if nano_sum == 0:
                    ...
                    isdone = True
        self.nano_iter = self.nano_mob*self.n_nano_placed
