...
def Initialize_Growth(seed,baseline=False):
    ...
    if baseline:
        ...
        #Right probe
        growth.nano[mid_probe:mid_probe+probe_width,-250:] = 1
        growth.fluid[mid_probe:mid_probe+probe_width,-250:] = 0
...
def Score_Growth_3D(chip,resolution = 25,flux_width=1.1,tmax=100,baseline=False,e_func=False):
    ...
    if not e_func:
        ...
        chip = (chip)*(epsilon_si-epsilon_air) + epsilon_air
        xspan = (chip.shape[0]+250*2)*lattice_size
        yspan = (chip.shape[1]+500*2)*lattice_size
        zspan = 2+h
    ...
    if baseline:
        ...
        scorer = bandpass_scorer(wvl)
        score = np.sum(scorer*Ts_out)
...
def bandpass_scorer(wavelength):
    scorer = np.zeros(wavelength.shape)-1
